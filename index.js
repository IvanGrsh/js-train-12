...
function createDictionary(entries) {
  ...
  entries.forEach(([key, value]) => create.set(key, value));
  return create;
...
function setValue(dictionary, key, value) {
  dictionary.forEach((values, keys) =>
    keys === key
      ? dictionary.set(key, values.toUpperCase())
...
function addKeyValuePairs(dictionary, entries) {
  ...
  entries.forEach(([key, value]) =>
    dictionary.has(key) !== true
      ? dictionary.set((key, value), added++)
      : rejected++
  );
...
function checkPresence(dictionary, keys) {
  let presence = [];
  keys.forEach((keys) => presence.push(dictionary.has(keys)));
  return presence;
